...
- **Migrations:** Drizzle Kit (`drizzle-kit generate` to produce SQL migration files from the schema) plus a small runner script (`npm run db:migrate`, see [§6](#6-migration-strategy)) to apply them — not the bare `drizzle-kit migrate` CLI, which has no way to guarantee InnoDB on a fresh database before migration `0000`'s foreign key is added.
...
- **Schema creation via Drizzle migrations.** Write the schema in `src/lib/db/schema.ts` (as sketched above), then run `drizzle-kit generate` to produce SQL migration files from schema diffs. No hand-written migration SQL for schema changes (the one exception is the InnoDB-engine migration below, which Drizzle's schema DSL has no way to express and must be hand-written via `drizzle-kit generate --custom`).
- **Apply migrations via `npm run db:migrate` (`scripts/migrate.ts`), not the bare `drizzle-kit migrate` CLI.** The script does exactly what the CLI does (loads `.env.local`, then calls the same `drizzle-orm/mysql2/migrator` `migrate()` function against `./drizzle`), plus one addition: it issues `SET SESSION default_storage_engine = InnoDB` on the connection first. The script opens its own dedicated single `mysql2` `Connection` for this — deliberately not the app's shared `Pool` from `src/lib/db/client.ts` — since a pool can lease a different physical connection per query, which would risk the `SET SESSION` landing on a connection `migrate()` never actually uses (a per-session setting silently not applying where it's needed). This closes a real gap the bare CLI has — migration `0000` creates `tasks`/`task_completions` with unqualified `CREATE TABLE` statements (relying on the server's default engine) before adding a foreign key with `ON DELETE CASCADE`; on a fresh MySQL server whose default isn't InnoDB, that foreign key statement fails and `0000` never completes, so migration `0001` (which converts the engine after the fact) never gets a chance to run either. Since `0000` is immutable once applied anywhere, and nothing can be inserted "before" it in a sequential migration history, the session-level engine override is the only fix that doesn't touch already-applied migration files: it makes `0000`'s own `CREATE TABLE` statements produce InnoDB tables regardless of the server's configured default, with no change to their SQL text. Harmless against an already-migrated database — nothing about session-level engine defaults affects tables that already exist, and every currently-applied migration is skipped anyway (see the next point).
- **Drizzle's MySQL migration tracking compares timestamps, not file hashes.** `readMigrationFiles`/`dialect.migrate` (in `drizzle-orm`) only checks each migration's journal timestamp against the single most recently applied timestamp recorded in `__drizzle_migrations` — a migration whose timestamp is at or before that is skipped outright, without re-reading or re-verifying its file content. This is why the session-level `SET` above is safe to add without touching `0000`/`0001`/`0002`'s files: on this project's already-migrated database, all three are skipped regardless, so the new statement has nothing to affect there — it only matters the one time a truly empty database runs every migration from `0000` onward.
